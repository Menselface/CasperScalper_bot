...
from infrastructure.db_pack.db import status_of_ending_of_registration
...
async def final_of_registration_date(bot: Bot):
    ...
    all_users = await status_of_ending_of_registration(
        thresholds["three_days_before"], thresholds["seven_day_after"]
    )
            ...
            if reg_date == threshold_date:
                if key in ["one_day_before", "today"]:
                    ...
                    date_str = date_str.replace(
                        english_month,
                        months_translation.get(english_month, english_month),
                    )
