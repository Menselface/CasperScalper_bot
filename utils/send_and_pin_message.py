...
from db_pack.db import get_all_id_with_registered_to_status, get_first_message
...
async def send_and_pin(bot: Bot):
    ...
    for user in all_users:
        ...
        day, month = await handle_stats(res, from_statistic=True, from_yesterday=yesterday)

        try:
            day_statistic_message = await bot.send_message(user, day, disable_notification=True)
            await bot.pin_chat_message(user, day_statistic_message.message_id, disable_notification=True)
            await bot.send_message(user, chat_link_message, disable_notification=True, disable_web_page_preview=True)
            await bot.send_message(user, support_community_message, disable_notification=True)
...
async def send_and_pin_month(bot: Bot):
    ...
    for user, result in zip(all_users, results):
        if isinstance(result, Exception):
            logger.warning(f"Ошибка при отправке/закреплении сообщения для пользователя {user}: {result}")
