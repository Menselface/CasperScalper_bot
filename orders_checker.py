...
from infrastructure.db_pack.db import (
    ...
    get_totalamountonpurchace_from_any_table,
    get_order_id_limit_from_any_table,
...
class OrdersChecker:
    ...
    async def get_asset_balance(
        self, account_info, pair, slice_, object_: str = "free"
    ):
    ...
    async def processing_not_found(self, not_founds):
        ...
        for pair, orders_list in not_founds.items():
            ...
            for record in orders_list:
                try:
                    order = await self.mexc.get_order_status(
                        order_id=record["order_id_limit"], symbol=pair
                    )
                    ...
                    if status == "CANCELED":
                        await delete_order_by_user_and_order_id_from_any_table(
                            table_name, self.user, record["order_id"]
                        )
                    ...
                    if "Order lost in the echo" in error_message:
                        logger.info(
                            f"Ордер попал в исключение - Удаляем и концы в воду.{record} \n{e}"
                        )
                        await delete_order_by_user_and_order_id_from_any_table(
                            table_name, self.user, record["order_id"]
                        )
    ...
    async def get_orders_from_exchange(self, user_id, start_time, end_time=None):
        ...
        for pair in PAIR_TABLE_MAP.keys():
            ...
            orders = await self.mexc.get_all_orders(
            ...
            for local_order in local_orders:
                ...
                if matching_order:
                    ...
                    if order_status and order_status != "NEW":
                        logger.info(
                            f"Ордер ID {local_order_id} для пары {pair} изменил статус на {order_status}."
                        )
    ...
    async def filter_active_orders(self, all_data):
        ...
        for pair, local_orders in all_data.items():
            ...
            if active_orders_from_exchange == "0":
                ...
                logger.info(
                    f"Нет открытых ордеров от биржи для пары {pair}. Оставляем локальные ордера без изменений."
                )
                continue
            ...
            logger.info(
                f"После фильтрации для пары {pair}: {len(filtered_data[pair])} ордеров осталось."
            )

        return filtered_data

    async def fill_orders_to_tables(
        self,
        user_id: int,
        orders_result: dict,
    ):
                ...
                try:
                    ...
                    if status == "FILLED":
                        ...
                        totalamountonpurchace = (
                            await get_totalamountonpurchace_from_any_table(
                                table_name, order_buy_id
                            )
                        )
                        fee = await CountOrderCommission(
                            self.user, order_buy_id, pair
                        ...
                        total_balance_usdt = await self.get_asset_balance(
                            account_info, pair, slice_last_4
                        )
                        total_open_trades = (
                            len(
                                await AccountMexcMethods(
                                    self.api_key, self.api_secret
                                ).get_open_orders()
                            )
                            or 0
                        )
                        btc_in_orders = await self.get_asset_balance(
                            account_info, pair, slice_first_3, "locked"
                        )
                        total_free_usdt = await self.get_asset_balance(
                            account_info, pair, slice_first_3, "free"
                        )
...
async def start_orders_checker(bot: Bot):
    ...
    for user in active_users:
        ...
        for pair, table in PAIR_TABLE_MAP.items():
            all_data[pair] = (
                await get_all_open_sell_orders_autobuy_from_any_table_for_checker(
                    user_id, pair
                )
            )
    ...
    for pair, orders_list in orders.items():
        ...
        try:
            for order in orders_list:
                ...
                res = await get_order_id_limit_from_any_table(
                    table_name, user_id, order
                )
