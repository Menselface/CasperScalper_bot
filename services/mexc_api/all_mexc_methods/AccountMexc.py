...
class AccountMexcMethods:
    ...
    async def get_order_status(self, order_id: str = None, orig_client_order_id: str = None, symbol: str = "KASUSDT"):
        ...
        try:
            ...
            if 'orderId' in result:
                return result
            elif "must be sent, but both were empty/null!" in  result['msg'] or result.get('code') == -2013:
    ...
    async def get_commission(self, order_id):
        params = {
            'symbol': self.symbol,
            'recvWindow': self.recv_window,
            'timestamp': int(time.time() * 1000),
        ...
        async with httpx.AsyncClient() as client:
            try:
                response = await client.get(f'{BASE_URL}/myTrades', headers=headers, params=params)
                response.raise_for_status()
