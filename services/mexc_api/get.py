...
class GetMexcAPI(BaseMexcAPI):
    def __init__(
        self, user_id: int, api_key: str, secret_key: str, recvwindow: int = 50000
    ):
    ...
    async def order_commission(self, symbol: str, order_id: str):
        self.params.update({"symbol": symbol, "orderId": order_id})
        self.return_params()

        async with httpx.AsyncClient() as client:
            response = await client.get(
                f"{BASE_URL}/myTrades", headers=self._get_headers(), params=self.params
            )
