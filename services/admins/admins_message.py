...
class AdminsMessageService:
    ...
    @classmethod
    @send_message_safe_call(default_return=[])
    async def send_admin_main_logger_file(
        cls, bot: Bot, msg: str = None, admin_id: int = None
    ):
            ...
            if not admin_id:
                for admin in await cls.get_all_admins_from_db():
                    await bot.send_document(
                        chat_id=admin, document=file, caption=msg if msg else "."
                    )
                await cls.remove_copy_main_logger_file()
            else:
                await bot.send_document(
                    chat_id=admin_id, document=file, caption=msg if msg else "."
                )
    ...
    @classmethod
    @send_message_safe_call(default_return=[])
    async def send_admin_user_logger_file(
        cls, user_id: int, symbol: str, bot: Bot, msg: str = None
    ):
