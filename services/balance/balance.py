...
class AssetBalance:
    ...
    def format_in_orders(self):
        return (
            f"{self.in_orders:.6f}" if self.symbol == "BTC" else f"{self.in_orders:.2f}"
        )
...
async def get_balance_data(user_id: int) -> dict:
    ...
    return {
        "usdt": {
            "available": acc_info.total_free_usdt,
            "in_orders": acc_info.usdt_locked,
        },
        "usdc": {
            "available": acc_info.total_free_usdc,
            "in_orders": acc_info.usdc_locked,
        },
        "kaspa": {
            "available": acc_info.free_kas,
            "in_orders": acc_info.locked,
            "for_total": acc_info.free_kas * await get_symbol_price("KASUSDT"),
        },
        "btc": {
            "available": acc_info.free_btc,
            "in_orders": acc_info.locked_btc,
            "for_total": acc_info.free_btc * await get_symbol_price("BTCUSDC"),
        },
        "sui": {
            "available": acc_info.free_sui,
            "in_orders": acc_info.locked_sui,
            "for_total": acc_info.free_sui * await get_symbol_price("SUIUSDT"),
        },
        "tao": {
            "available": acc_info.free_tao,
            "in_orders": acc_info.locked_tao,
            "for_total": acc_info.free_tao * await get_symbol_price("TAOUSDT"),
        },
        "pyth": {
            "available": acc_info.free_pyth,
            "in_orders": acc_info.locked_pyth,
            "for_total": acc_info.free_pyth * await get_symbol_price("PYTHUSDT"),
        },
        "dot": {
            "available": acc_info.free_dot,
            "in_orders": acc_info.locked_dot,
...
async def get_order_data(user_id: int, symbol: str) -> dict:
    ...
    all_orders = await get_all_open_sell_orders_autobuy_from_any_table_for_checker(
        user_id, symbol
    )
    ...
    sell_sum = sum(
        (order["priceordersell"] or 0) * (order["qtytosell"] or 0)
