...
class CountOrderCommission(OrdersCheckerUtils):
    ...
    async def get_maker_commission(self):
        limit_order_id = await self.select_from_orders.limit_order_id_by_order_id(self.symbol, self.order_id, self.user_id)
        if not limit_order_id:
            logger.warning(f"[{self.__class__.__name__} maker] No limit_order_id for {self.order_id}")
            return 0.0
