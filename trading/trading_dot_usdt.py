...
from infrastructure.db_pack.repositories.trading_repo.any_table import GetOrdersAnyTable, UpdateOrdersAnyTable
...
from trading.db_querys.db_methods_for_sui import (
    delete_order_by_user_and_order_id_from_any_table_by_symbol,
)
...
async def dot_trader(message: Message, bot: Bot, result: dict = None):
    ...
    while True:
        is_user_expired = await validate_user_status(
            message, user_id, symbol=symbol_name, manager=manager_dot, bot=bot
        )
        if is_user_expired:
        ...
        if (
            ...
            dot_limit = await get_user_symbol_data(user_id, symbol_name, "trade_limit")
            logs.order_limits_by_and_trade_limit_user(order_limit_by_user, dot_limit)
            ...
            order_buy_id, qnty_for_sell, price_to_sell = (
                await buy_sell.open_market_order_buy(bot=bot)
            )
            dict_data = {
            ...
            if price_to_sell == "critical_error":
                ...
                return
            result = await buy_sell.open_limit_order_sell(
        ...
        while True:
            ...
            if is_user_expired:
                return
            dot_price = await get_symbol_price(symbol_name)
            ...
            if float(dot_price) <= float(threshold_price):
                await message.answer(
                    f"🔻 <b>УВЕДОМЛЕНИЕ</b> 🔻 цена\n️DOT упала до {round(threshold_price, 3)} (на {round(auto_buy_down_perc, 2)} % от {round(avg_price, 3)})"
                ).as_(bot)
                result = None
...
async def orders_checker(
    ...
    user_orders_from_table = await get_all_open_sell_orders_autobuy_from_any_table(
        user_id, symbol, 1
    )
    closed_orders = set()
    ...
    for record in user_orders_from_table:
        try:
            ...
            if status == "FILLED":
                ...
                await update_symbol_orders_table.update_order_after_sale_by_order_id_limit(
                    repo=symbol,
                    user_id=int(user_id),
            ...
            if status == "CANCELED":
                await delete_order_by_user_and_order_id_from_any_table_by_symbol(
                    symbol, user_id, current_order_id
                )
                user_logger.info(
                    f"User {user_id} canceled order with status 1 DOT/USDT {order}"
                )
                result.update({"autobuy_was_cancelled": True})

        except Exception as e:
            user_logger.info(f"Ордер DOT/USDT попал в исключение - {e}")
            continue
        else:
            pass
...
async def send_messages_to_user(message: Message, orders, bot, user_logger, symbol):
    ...
    for limit_order_id in orders:
        try:
            res = await select_any_table.select_id_limit_details_of_order(
                symbol, limit_order_id, user_id
            )
            ...
            user_message = user_message_returner(
                qnty_for_sell, price_to_sell, total_after_sale, fee_limit_order, symbol
            )
