...
async def tao_trader(message: Message, bot: Bot, result: dict = None):
    ...
    while True:
        is_user_expired = await validate_user_status(
            message, user_id, symbol=symbol_name, manager=manager_tao, bot=bot
        )
        if is_user_expired:
        ...
        if (
            ...
            tao_limit = await get_user_symbol_data(user_id, symbol_name, "trade_limit")
            logs.order_limits_by_and_trade_limit_user(order_limit_by_user, tao_limit)
        ...
        while True:
            ...
            if is_user_expired:
                return
            tao_price = await get_symbol_price(symbol_name)
            ...
            if float(tao_price) <= float(threshold_price):
                await message.answer(
                    f"🔻 <b>УВЕДОМЛЕНИЕ</b> 🔻 цена\n️TAO упала до {round(threshold_price, 4)} (на {round(auto_buy_down_perc, 2)} % от {round(avg_price, 4)})"
                ).as_(bot)
                result = None
        ...
        try:
            ...
            if status == "FILLED":
                ...
                await update_symbol_orders_table.update_order_after_sale_by_order_id_limit(
                    ...
                    symbol_in_orders=tao_in_orders,
