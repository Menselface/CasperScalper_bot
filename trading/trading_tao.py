...
async def tao_trader(message: Message, bot: Bot, result: dict = None):
    ...
    """Есть команда  СТОП?"""
    while True:
        is_user_expired = await validate_user_status(message, user_id, symbol=symbol_name, manager=manager_tao,
                                                     bot=bot)
        if is_user_expired:
        ...
        if result and result["avg_price"] and not user_get_limit_or_balance_message_status:
            ...
            if await trade_util.if_not_start_stop_at_symbols_for_trade(user_id, manager_tao, logs):
                return
            is_user_expired = await validate_user_status(message, user_id, symbol=symbol_name, manager=manager_tao,
                                                         bot=bot)
            if is_user_expired:
