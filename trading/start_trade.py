...
user_start_trade_router = Router()


@user_start_trade_router.callback_query(StartTrade.filter())
async def user_set_up_callbacks(
    callback: CallbackQuery, callback_data: StartTrade, bot: Bot, state: FSMContext
):
...
async def user_restart_from_admin_panel(message: Message, bot: Bot):
    ...
    if kaspa:
        ...
        tasks.append(task)
    if sui:
        data = await get_all_open_sell_orders_autobuy_from_any_table(
            user_id, "SUIUSDT", status=1
        )
        result = await process_order_result(data)
