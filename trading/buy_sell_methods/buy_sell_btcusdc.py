...
from infrastructure.db_pack.db import get_info_commission_percent
from services.mexc_api.all_mexc_methods.mexc_methods import CreateSpotConn
from trading.db_querys.db_for_btc_table import (
    update_all_not_autobuy,
    set_order_buy_in_db,
    update_order_by_order_id,
    get_buy_price,
    spend_in_usdt_for_buy_order,
)
...
class BuySellOrders:
    ...
    async def open_kaspa_market_order_buy(self, bot, buy_market: bool = False):
        ...
        try:
            ...
            logger.info(
                f"Пользователь {self.user_id}: Открытие нового ордера {order_buy_id}"
            )
            order_buy_details = await asyncio.to_thread(
                self.http_mexc.order_details, order_buy_id
            )
            ...
            if "Insufficient position" in str(e):
                ...
                logger.warning(
                    f"Too Many Requests для пользователя {self.user_id} {self.symbol}"
                )
                await asyncio.sleep(2)
                return None, None, "Error 429"
            else:
                logger.critical(
                    f"Ошибка у пользователя {self.symbol} {self.user_id}: {str(e)}"
                )
                logger.critical("Подробности ошибки:\n" + traceback.format_exc())
                logger.opt(exception=True).critical("Детальный стек вызовов")
                await notify_admin(
                    user_id=self.user_id, symbol=self.symbol, error_msg=str(e), bot=bot
                )
                return None, None, "critical_error"

    async def open_kaspa_limit_order_sell(
        self, user_id, order_buy_id, qnty_for_sell, price_to_sell, bot, buy_market=False
    ):
        while True:
            try:
                ...
                logger.info(
                    f"Пользователь {self.user_id}: Открытие нового ордера продажа {order_sell_id}"
                )
                order_sell_details = self.http_mexc.order_details(order_sell_id)
                time_of_order_sell = await create_time(
                    order_sell_details.get("updateTime")
                )
                ...
                await update_order_by_order_id(
                    int(user_id),
                    str(order_buy_id),
                    time_of_order_sell,
                    float(qnty_for_sell),
                ...
                spend_in_usdt_for_buy = await spend_in_usdt_for_buy_order(
                    user_id, order_sell_id
                )
                message_order_buy = (
                    f"<b>КУПЛЕНО:</b>\n"
