...
async def sui_trader(message: Message, bot: Bot, result: dict = None):
    ...
    is_user_expired = await validate_user_status(
        message, user_id, symbol=symbol_name, manager=manager_sui, bot=bot
    )
        ...
        while True:
            ...
            if float(sui_price) <= float(threshold_price):
                await message.answer(
                    f"🔻 <b>УВЕДОМЛЕНИЕ</b> 🔻 цена\n️SUI упала до {round(threshold_price, 4)} (на {round(auto_buy_down_perc, 2)} % от {round(avg_price, 4)})"
                ).as_(bot)
                result = None
