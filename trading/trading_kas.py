...
from db_pack.db import get_access_key, get_secret_key, delete_order_by_user_and_order_id, update_orderafter_sale_by_order_id, \
    get_all_open_sell_orders_autobuy, \
    get_orders_from_data, get_info_commission_percent
...
async def kaspa_trader(message: Message, bot: Bot, result: dict = None):
    ...
    is_user_expired = await validate_user_status(message, user_id, symbol=symbol_name, manager=manager_kaspa,
                                                 bot=bot)
    ...
    while True:
        if result and result["avg_price"] and not user_get_limit_or_balance_message_status:
            ...
            if await trade_util.if_not_start_stop_at_symbols_for_trade(user_id, manager_kaspa, logs):
                return
            is_user_expired = await validate_user_status(message, user_id, symbol=symbol_name, manager=manager_kaspa,
                                                         bot=bot)
            if is_user_expired:
            ...
            kaspa_limit = await get_user_symbol_data(user_id, symbol_name, "trade_limit")
        ...
        while True:
            is_user_expired = await validate_user_status(message, user_id, symbol=symbol_name, manager=manager_kaspa, bot=bot)
            if is_user_expired:
                return
