...
from infrastructure.db_pack.repositories.trading_repo import (
...
from trading.db_querys.db_for_btc_table import (
    ...
    get_all_open_sell_orders_autobuy_btc,
)
...
async def btc_usdc_trader(message: Message, bot: Bot, result: dict = None):
    ...
    while True:
        is_user_expired = await validate_user_status(
            message, user_id, symbol=symbol_name, manager=manager_btc, bot=bot
        )
        if is_user_expired:
        ...
        if (
            ...
            btc_limit = await get_user_symbol_data(user_id, symbol_name, "trade_limit")
            logs.order_limits_by_and_trade_limit_user(order_limit_by_user, btc_limit)
            ...
            if user_buy_stats + order_limit_by_user >= btc_limit:
                ...
                if user_get_limit_message:
                    ...
                    await bot.send_message(
                        chat_id=user_id,
                        text=f"Установленный вами лимит {safe_format(order_limit_by_user, 2)} по BTC/USDC достиг {safe_format(btc_limit, 2)} USDC\n Вы можете изменить его в /parameters",
                    )
        ...
        while True:
            ...
            if is_user_expired:
                return
            btc_price = await get_symbol_price(symbol_name)
            ...
            if float(btc_price) <= float(threshold_price):
                await message.answer(
                    f"🔻 <b>УВЕДОМЛЕНИЕ</b> 🔻 цена\nBTC упала до {safe_format(threshold_price, 2)} (на {safe_format(auto_buy_down_perc, 2)} % от {safe_format(avg_price, 2)})"
                ).as_(bot)
                result = None
        ...
        try:
            ...
            if status == "FILLED":
                ...
                await update_symbol_orders_table.update_order_after_sale_by_order_id_limit(
                    ...
                    symbol_in_orders=btc_in_orders,
