...
from services.parameters.utils_parameters import ParametersCreateKeyboard
...
async def params_choice_symbol(user_id: int):
    keyboard = InlineKeyboardBuilder()
    for (
        text,
        callback,
    ) in await ParametersCreateKeyboard().get_symbols_for_main_keyboard(user_id):
        keyboard.button(
            text=text,
            callback_data=ParamsMyCallbackSymbol(level=1, action=callback).pack(),
        )
...
async def params_keyboard(user_id, symbol, for_everything: bool = False):
    keyboard = InlineKeyboardBuilder()
    ...
    if not for_everything:
        ...
        if isinstance(autobuy_down_percent, float) and autobuy_down_percent != 1000:
            keyboard.button(
                text=f"Процент при падении – {safe_format(autobuy_down_percent, 2)} %",
                callback_data=ParamsMyCallbackSymbol(
...
async def user_set_up_keyboard(user_id):
    ...
    btc_status = next(
        (currency.get("BTCUSDC") for currency in user_data if "BTCUSDC" in currency),
        False,
    )
    kaspa_status = next(
        (currency.get("KASUSDT") for currency in user_data if "KASUSDT" in currency),
        False,
    )
    sui_status = next(
        (currency.get("SUIUSDT") for currency in user_data if "SUIUSDT" in currency),
        False,
    )
    pyth_status = next(
        (currency.get("PYTHUSDT") for currency in user_data if "PYTHUSDT" in currency),
        False,
    )
    dot_status = next(
        (currency.get("DOTUSDT") for currency in user_data if "DOTUSDT" in currency),
        False,
    )
    tao_status = next(
        (currency.get("TAOUSDT") for currency in user_data if "TAOUSDT" in currency),
        False,
    )
...
def create_pagination_keyboard(
    current_page: int, total_pages: int, pair: str
    ...
    builder.button(
        text=f"({current_page}/{total_pages})", callback_data="disabled", disabled=True
    )
