...
async def params_choice_symbol(user_id: int):
    ...
    for text, callback in await ParametersCreateKeyboard().get_symbols_for_main_keyboard(user_id):
        keyboard.button(
            text=text,
...
async def params_keyboard(
    ...
    if not for_everything:
        ...
        keyboard.button(text=f'Размер ордера – {safe_format(user_order_limit, 1)} USDT', callback_data=ParamsMyCallbackSymbol(level=1, action=f"set_order_limit_{symbol}").pack())
        keyboard.button(text=f'Процент прибыли – {safe_format(percent_profit, 2)} %', callback_data=ParamsMyCallbackSymbol(level=1, action=f"set_profit_percent_{symbol}").pack())
        ...
        if autobuy_down_percent == 1000:
            keyboard.button(text=f'Процент при падении – откл. 🚫',
                            callback_data=ParamsMyCallbackSymbol(level=1, action=f"set_autobuy_down_{symbol}").pack())
        keyboard.button(text=f'Лимит на покупку {symbol_str} {"♾️" if user_trade_limit == 1000000 else user_trade_limit}', callback_data=ParamsMyCallbackSymbol(level=1, action=f"limit_of_trading_{symbol}").pack())
        
        keyboard.button(text='Установить по умолчанию', callback_data=ParamsMyCallbackSymbol(level=1, action=f"reset_settings_{symbol}").pack())
