...
from bot.keyboards import (
    params_keyboard,
    user_autobuy_down_keyboard_off,
    back_keyboard,
    yes_no_keyboard,
    params_choice_symbol,
    ParamsMyCallbackSymbol,
)
from trading.db_querys.db_symbols_for_trade_methods import (
    get_user_symbol_data,
    update_user_symbol_data,
    set_standart_user_params,
    set_standart_user_params_for_all,
    get_user_exist_with_symbol,
    add_user,
)
from utils.additional_methods import format_symbol, safe_format
from utils.decorators import parameters_safe_call
...
@parameters_router.message(Command("parameters"))
@parameters_router.callback_query(StateFilter("*"), F.data == "cancel_to_parametrs")
@parameters_safe_call(default_return=[])
async def handle_parameters_choice_symbol(
    ...
    await bot.send_message(
        ...
        reply_markup=await params_choice_symbol(user_id),
    )


@parameters_router.callback_query(ParamsMyCallbackSymbol.filter())
@parameters_safe_call(default_return=[])
async def user_choice_symbol_in_params(
    ...
    if action.startswith("set_order_limit_"):
        mes = await callback.message.answer(
            f"Введите новый размер ордера ({text_area_symbol}):",
    ...
    if action.startswith("set_profit_percent_"):
        mes = await callback.message.answer(
            "Введите новый процент прибыли (%):", reply_markup=await back_keyboard()
        )
        await state.update_data(message_id_callback=mes.message_id)
    ...
    if action.startswith("limit_of_trading_"):
        ...
        mes = await callback.message.answer(
            f"Введите лимит покупки для пары {text_area_symbol}",
            reply_markup=await back_keyboard(),
        )
        await state.update_data(message_id_callback=mes.message_id)
        await state.set_state(ParametersState.user_trade_limit)
    ...
    if action.startswith("set_autobuy_down_"):
        user_id = callback.from_user.id
        autobuy_status = await get_user_symbol_data(
            user_id, symbol, "auto_buy_down_perc"
        )
        if not autobuy_status:
            result = 0
            for pair in PAIR_TABLE_MAP:
                result += await get_user_symbol_data(
                    user_id, pair, "auto_buy_down_perc"
                )
            if result == 4000:
                mes = await callback.message.answer(
                ...
                mes = await callback.message.answer(
                    "Введите процент для автопокупки при падении (%):",
    ...
    if action.startswith("reset_settings_"):
        mes = await callback.message.answer(
            f"Сбросить параметры на стандартные для пары {text_area_symbol}?\n"
...
@parameters_router.callback_query(
    StateFilter(ParametersState.autobuy_down_percent), F.data == "autobuy_off"
)
@parameters_safe_call(default_return=[])
async def autobuy_off(callback_query: types.CallbackQuery, state: FSMContext, bot: Bot):
    ...
    if text_area_symbol == "everything":
        ...
        mes = await callback_query.message.answer(
            f"Процент покупки для {text_area_symbol} при падении отключен ✅\n"
        )
        return
    await state.set_state(None)
    user_id = callback_query.from_user.id
...
async def process_order_limit(message: types.Message, state: FSMContext, bot: Bot):
    ...
    for sym in symbols_to_update:
        await update_user_symbol_data(
            user_id, sym, order_limit_by=round(user_new_limit)
        )
...
async def process_percent_profit(message: types.Message, state: FSMContext, bot: Bot):
    ...
    message_id = data.get("message_id")
    message_id_callback = data.get("message_id_callback")
    if not (0.2 <= user_new_limit < 100.0):
    ...
    for sym in symbols_to_update:
        await update_user_symbol_data(
            user_id, sym, percent_profit=round(user_new_limit, 2)
        )
...
async def process_user_trade_limit(message: types.Message, state: FSMContext, bot: Bot):
    ...
    if not user_new_limit:
        await message.answer(
            "Пожалуйста введите значение только <b>целыми</b> числами."
        )
...
async def process_autobuy_down(message: types.Message, state: FSMContext, bot: Bot):
    ...
    if not user_new_limit >= 0.3:
        await message.answer(
            "Минимальный процент падения может составлять 0.3 %, пожалуйста введите корректное число:"
        )
        return
    if symbol == "everything":
        text_area_symbol = "Для всех торговых пар"
        ...
        user_old_parametr = await get_user_symbol_data(
            user_id, symbol, "auto_buy_down_perc"
        )
        text_area_symbol = format_symbol(symbol)
        symbols_to_update = [symbol]
    ...
    for sym in symbols_to_update:
        await update_user_symbol_data(
            user_id, sym, auto_buy_down_perc=round(user_new_limit, 2)
        )

    await message.delete()

    logger.info(
        f"Пользователь {user_id}: Процент падения (%) изменен: {user_new_limit}"
    )
