...
order_status_router = Router(name=__name__)


@order_status_router.message(Command("orders"))
async def handle_status_command(message: Message):
    await message.answer(
        "Выберите валютную пару:", reply_markup=create_pair_selection_keyboard()
    )


@order_status_router.callback_query(PairCallback.filter())
async def handle_pair_selection(
    callback: CallbackQuery, callback_data: PairCallback, state: FSMContext, bot: Bot
):
    pair = callback_data.pair
    user_id = callback.from_user.id
    await callback.answer(f"Вы выбрали пару {pair}. Загружаю ордера...", cache_time=4)

    all_data_from_db = await get_all_open_sell_orders_autobuy_from_any_table(
        user_id, pair, 1
    )
    all_data_from_db_with_nine = await get_all_open_sell_orders_autobuy_from_any_table(
        callback.from_user.id, pair, 9
    )

    sorted_records = sorted(
        all_data_from_db, key=lambda x: x["transacttimebuy"], reverse=True
    )
    sorted_records.extend(all_data_from_db_with_nine)
    grouped_records = [
        sorted_records[i : i + 5] for i in range(0, len(sorted_records), 5)
    ]
...
async def send_paginated_message(
    ...
    for record in records:
        ...
        totalsellamount = (
            f"{float(qtytosell) * float(priceordersell):.3g}"
            if qtytosell and priceordersell
            else "0"
        )
        ...
        if pair == "BTCUSDC":
            messages.append(
                ...
                f"- Куплено по {safe_format(priceorderbuy, 6)} ({safe_format(totalamountonpurchace, 2)} USDC)\n"
                f"- Продается по {safe_format(priceordersell, 6)} ({safe_format(totalsellamount, 2)} USDC)\n"
                f"{formatted_date}"
            )
        else:
            messages.append(
                f"{number}. Ордер на продажу\n"
                f"{safe_format(qtytosell, 6)} {format_symbol(pair)}\n"
                f"- Куплено по {safe_format(priceorderbuy, 2)} ({safe_format(totalamountonpurchace, 2)} USDT)\n"
                f"- Продается по {safe_format(priceordersell, 2)} ({safe_format(totalsellamount, 2)} USDT)\n"
                f"{formatted_date}"
            )

        number += 1
    header = (
        f"{format_symbol(pair)}    {' ✅ торгует' if is_user_trade_now else '⬛️ OFF'}"
        if current_page == 1
        else ""
    )
    result_text = (
...
@order_status_router.callback_query(lambda callback: callback.data.startswith("page:"))
async def handle_pagination(callback: CallbackQuery, state: FSMContext, bot: Bot):
    _, page, pair = callback.data.split(":")
    page = int(page)

    all_data_from_db = await get_all_open_sell_orders_autobuy_from_any_table(
        callback.from_user.id, pair, 1
    )
    all_data_from_db_with_nine = await get_all_open_sell_orders_autobuy_from_any_table(
        callback.from_user.id, pair, 9
    )
