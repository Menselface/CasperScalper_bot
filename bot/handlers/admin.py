...
from db_pack.db import is_admin_checker, set_reset_autobuy, get_all_id_with_registered_to_status
    ...
    if not arg:
        ...
        for user in all_users:
            try:
                ...
                logger.warning(f"Ошибка при отправке сообщения пользователю {user} - {e}")
                not_delivered.append(user)
    if not not_delivered:
        await bot.send_message(user_id, f"Сообщение отправлено пользователям - {delivered}")
        return
    else:
